<template lang='pug'>
  div(v-if="!userIsShareholder")
    v-button.zerox-popup(@click='showInfoPopup') donate in our Company
    popup(
      ref='investorInfoPopup'
      title='donate now in our company'
    )
      form-row(no-inputs)
        p You can be a part of our donate-company. Click the button below to buy our tokens and to claim your slice of our pie!
      form-row(no-inputs v-if="userBalance==='0.0'")
        p You don't have any methereum, buy some first!
        div.button-area
  ...
  methods: {
    renderZeroExInstant() {
      ...
      const signedOrders = [{
        ...
        makerAddress: '0xBDcda71cde2B2481fA2f8756c497291b2f60e374',
