...
contract('Pie', async function(accounts) {
  let pie
  ...
  before(done => {
    ;(async () => {
      try {
        ...
        // Deploy Pie.sol
        pie = await Pie.new("Test")
        var tx = await web3.eth.getTransactionReceipt(pie.transactionHash)
        totalGas = totalGas.plus(tx.gasUsed)
        console.log(_ + tx.gasUsed + ' - Deploy pie')
